...
siguientes\+De\+: secu(value\+\_\+type) s x value\+\_\+type v $\to$ secu(value\+\_\+type) \{esta?(v,s)\}~\newline
 siguientes\+De(s, v) $\equiv$ {\bfseries if} prim(s) == v {\bfseries then} $<$$>$ {\bfseries else} prim(s) $\bullet$ \href{axiomas.html#siguientesDe}{\tt siguientes\+De}(s,fin(v)) {\bfseries fi} 
\end{DoxyParagraph}
...
Retorna la secuencia ordenada de elementos mayores al pasado como parametro. elementos\+Mayores\+A(d, c, k) $\equiv$ {\bfseries if} $\emptyset$(c)? {\bfseries then} $<$ $>$ {\bfseries else} {\bfseries if} maximo(c) $<$ k {\bfseries then} elementos\+MayoresA(d, c -\/ maximo(c), k) o $<$maximo(c), obtener(minimo(c), d)$>$ {\bfseries else} elementos\+MayoresA(d, c -\/ maximo(c), k) {\bfseries fi} {\bfseries fi} elementos\+MayoresA\+: deicc(value) x conj(key) x key $\to$ secu(value)~\newline
...
elementos\+MenoresA\+: dicc(value) x conj(key) x key $\to$ secu(value)~\newline
 elementos\+Menores\+A(d, c, k) $\equiv$ {\bfseries if} $\emptyset$(c)? {\bfseries then} $<$ $>$ {\bfseries else} {\bfseries if} minimo(c) $<$ k {\bfseries then} $<$minimo(c), obtener(minimo(c), d)$>$ $\bullet$ elementos\+MenoresA(d, c -\/ minimo(c), k) {\bfseries else} elementos\+MenoresA(d, c -\/ minimo(c), k) {\bfseries fi} {\bfseries fi} 
\end{DoxyParagraph}
...
\begin{DoxyRefDesc}{Obsoleto}
\item[\hyperlink{deprecated__deprecated000026}{Obsoleto}]Esta operación (y bastantes otras) son muy dificiles de leer 
...
\begin{DoxyRefDesc}{Obsoleto}
\item[\hyperlink{deprecated__deprecated000027}{Obsoleto}]sin\+Repetidos(header\+To\+Secu(n.\+parent)) esto no te soluciona uno de los problemas hablado por Soulignac. Piensen como podrían reescribir a partir del rep de map V\+I\+S\+TO\end{DoxyRefDesc}
