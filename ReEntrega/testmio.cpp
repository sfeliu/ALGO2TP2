...
/////////////////////////////
// Tests agregados por Teo //
////////////////////////////

struct TeoInstances : public ::testing::Test
{
	virtual void SetUp()
	{
		singleton.insert({1, "uno"});
...
TEST_F(TeoInstances, REND){
	if (ABORT) {
		auto rit=cinco_elementos.rend();
		auto it=cinco_elementos.begin();
		rit--;
		while (rit!=cinco_elementos.rbegin()){
			EXPECT_EQ(*it, *rit);
			rit--;
			it++;
		}
...
TEST_F(TeoInstances, AT_CONST){
		if (ABORT){
		const aed2::map<int, std::string> cinco_const(cinco_elementos);

		//EXPECT_EQ("uno", cinco_const.at(1));
		//EXPECT_EQ("dos", cinco_const.at(2));
	}
}

TEST_F(TeoInstances, FIND_CONST){
		if (ABORT){
		const aed2::map<int, std::string> cinco_const(cinco_elementos);

		EXPECT_EQ("uno", (*cinco_const.find(1)).second);
		EXPECT_EQ("dos", (*cinco_const.find(2)).second);
		EXPECT_EQ("cinco", (*cinco_const.find(5)).second);

	}
}

TEST_F(TeoInstances, ITERATE){
		if (1){
		auto it=cinco_elementos.begin();
		auto it2=cinco_elementos_std.begin();


		while(it!=cinco_elementos.end() && it2!=cinco_elementos_std.end()){
			EXPECT_EQ(*it, *it2);
			it++;
			it2++;
		}
...
TEST_F(TeoInstances, COMPLICATED){

	std::vector<int> vec;
	for (int i=0; i<25; i++) vec.push_back(i);

	if (ABORT){
		aed2::map<int, int> v5_elems;
		for (int i=0; i<25; i++){
			v5_elems.insert(std::make_pair(i,vec[i]));

		}

		auto itv=vec.begin();
		for(auto it=v5_elems.begin(); it!= v5_elems.end(); it++){
			ASSERT_EQ((*it).second, *itv);
			itv++;
		}
...
TEST_F(TeoInstances, COPY_CONSTR){
		if (1){
		aed2::map<int, std::string> cinco_copia (cinco_elementos);

		auto it=cinco_copia.begin(), it2=cinco_elementos.begin();

		for (int i=0;i<5;i++){
			ASSERT_EQ(*it, *it2);
		}

		ASSERT_TRUE((*cinco_copia.find(3)).second=="tres");
...
TEST_F(TeoInstances, HINT_HEAD){
	if (1){
		cinco_elementos.insert(cinco_elementos.end(), std::make_pair(8, "ocho"));
		cinco_elementos.insert(cinco_elementos.end(), std::make_pair(7, "siete"));


		EXPECT_EQ("siete", cinco_elementos.at(7));
		//EXPECT_EQ("dos", cinco_const.at(2));
	}
}

TEST_F(TeoInstances, DELFUP_NULL){
    if (ABORT){
        aed2::map<int, int> prueba;
        auto it=prueba.insert(std::make_pair(5, 5));
        prueba.insert(std::make_pair(8, 8));
        prueba.insert(std::make_pair(3, 3));
        prueba.insert(std::make_pair(1, 1));
        prueba.insert(std::make_pair(2, 2));


        EXPECT_EQ(8, prueba.at(8));
        EXPECT_EQ(3, prueba.at(3));
